...
```json
{
  ...
  "version":"2.0.0",
  "model":{"mode":"trained|fallback","source":"joblib|in-repo-default","path":".../models/deployment/gait_emotion_api_model.joblib"}
}
```
...
모델 파일이 있으면 `trained` 모드, 없거나 손상되면 자동으로 `fallback` 모드로 동작합니다.

---

## 7) 데이터셋/모델/사전학습 파일 요구사항

### API 실행에서 선택적으로 사용하는 파일

- `models/deployment/gait_emotion_api_model.joblib`
  - API가 시작될 때 로드 시도
  - 유효하면 `trained` 모드 사용
  - 없거나 손상되면 서버는 `fallback`(demo) 모드로 계속 동작
  - 실제 학습 모델로 교체하려면 동일 경로에 유효한 `joblib` 모델을 배치 후 서버를 재시작
...
### Q2. 학습 모델 없이도 예측이 되나요?

됩니다. 현재 기본 동작은 `fallback`(demo) 모드이며, `/predict_emotion`과 `/health`의 `model.mode`로 현재 모드를 확인할 수 있습니다.

학습 모델(`trained`)로 전환하려면:
- `models/deployment/gait_emotion_api_model.joblib`에 유효한 모델 파일 배치
- 서버 재시작
- `/health` 또는 예측 응답에서 `model.mode == "trained"` 확인
...
3. **예측 API는 모델 파일이 없어도 `fallback` 모드로 동작합니다.**
...
- **학습 모델 경로(선택):** `models/deployment/gait_emotion_api_model.joblib`
...
- **예측 API는 모델 파일이 없어도 fallback 모드로 동작한다.**
